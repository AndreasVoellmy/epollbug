...
Description: This program is a variation on kqueueserver4.c. It registers
each socket on all of the workers queues and always re-registers on all 
workers queues.
 */
...
// (4) Compile with: gcc -O2 kqueueserver6.c -lpthread -Wall -o kqueueserver6
// (5) To run: ./kqueueserver6
...
void receiveLoop(int sock, int epfd, int w, char recvbuf[]) {
  ...
  while(1) {
	...
	if (errno==EAGAIN) {  // re-arm the socket with epoll.
	  ...
	  for (i=0; i < NUM_WORKERS; i++) {
	    if (queue[i] != -1) {
	      if (kevent(queue[i], &event, 1, NULL, 0, NULL)) {
		perror("rearm");
		exit(-1);
	      }
